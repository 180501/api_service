  ...
  const columns: ProColumns<API.InterfaceInfo>[] = [
      {
        title: 'id',
        dataIndex: 'id',
        valueType: 'text',//id 列的属性定义为了index。这样定义的结果是，id 列不会出现在表单项里，也就不会被填入表单。但是改为text类型，就可以编辑了。
      },
      {
        title: '接口名称',
        //name对应后端的字段名
      ...
      <ProTable<API.RuleListItem, API.PageParams>
        ...
        request={async (params, sort: Record<string, SortOrder>, filter: Record<string, (string | number)[] | null>)=>{
          const res = await listInterfaceInfoByPageUsingPost({
            ...params,});
          if(res?.data){
            // alert(`res.data.total: ${res.data.total}, typeof res.data.total: ${typeof res.data.total}`);
            return {
              data: res?.data.records || [],
              total: Number(res?.data.total), // 确保total是数字类型
              success: res?.code === 0,
            };
          }else{//不能只有if返回值
            return {
